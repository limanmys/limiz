...
// AllowedPlugins is set at compile time via -ldflags.
// Comma-separated plugin names: "dir-size,gpu-info,services"
// If empty, no plugins will run.
var AllowedPlugins string

// isAllowedPlugin checks whether the given plugin name is in the
...
// VerifyPlugin performs two-layer security verification:
...
func VerifyPlugin(name, binaryPath string) error {
	...
	if !isAllowedPlugin(name) {
		return fmt.Errorf("plugin '%s' is not defined in the allowlist — binary must be recompiled", name)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to read plugin binary '%s': %v", binaryPath, err)
	}
